...
let lightInitialPositions = [vec3.fromValues(2, 0, 1), vec3.fromValues(-2, 0, 1)];
...
let fragmentShader = `
    ...
    {        
        ...
        outColor = texture(cubemap, reflectedDir) * texture(tex, vUv) + calculateLights(normalize(vNormal), vPosition);
...
let vertexShader = `
    ...
    void main()
    {
        ...
        vUv = uv;
        viewDir = (modelMatrix * position).xyz - camPos;                
...
(async () => {
    ...
    function renderReflectionTexture()
    {
        ...
        let reflectionCameraPosition = vec3.transformMat4(vec3.create(), camPos, reflectionMatrix);
    ...
    function drawObjects(cameraLocation, viewMatrix) {
        ...
        drawCall.uniform("cameraPosition", cameraLocation);
        drawCall.uniform("viewProjectionMatrix", viewProjMatrix);
        drawCall.uniform("modelMatrix", modelMatrix);
        drawCall.uniform("normalMatrix", mat3.normalFromMat4(mat3.create(), modelMatrix));
        drawCall.uniform("ambientLightColor", ambientLightColor);
    ...
    function draw() {
        let time = new Date().getTime() * 0.001 - startTime;

        mat4.perspective(projMatrix, Math.PI / 2.5, app.width / app.height, 0.1, 100.0);
        vec3.rotateY(camPos, vec3.fromValues(0, 3, 3.5), vec3.fromValues(0, 0, 0), time * 0.05);
